...
The pseudocode in a GPU is outlined in algorithm \ref{alg:interp}.  One thread per X-ray is launched to compute $N_{ray}$ threads organized in divV$\times$divU blocks.  This means that instead of computing each of the path integrals in lexicographical order, small subsets of blocks (in the detector) are computed together.  This decreases the memory latency and increases the overall speed of the kernels by up to 300\% in our tests.  For more information about GPU memory access and optimal X-ray indexing we refer the reader to the work by Chou \emph{et al}\cite{chou2011fast}.  As information about the texture cache is proprietory, empirical tests were made to find the best size for divV and divU.  These showed 32$\times$32 to be fastest on an NVIDIA Tesla 40k.  Note that there are reportedly faster structures for GPU kernels\cite{chou2011fast}, but our tests showed no such improvement so we have stuck to the simplest approach of one thread per ray.
...
%\begin{figure}[h]
%\begin{center}
%\includegraphics[width=0.7\linewidth]{fakethorax1.png} 
%\caption{\label{fig:fakethorax} Synthetic thorax-like phantom resembling two lungs and a tumour.} 
%\end{center}
%\end{figure}
...
\begin{eqnarray}
V(x,y,z) = \bigl(\nonumber\\
8t\sin(x \cdot \pi/L)\sin(y \cdot \pi/L)\sin(z \cdot \pi/L),\nonumber \\
8t\sin(x \cdot \pi/L)\sin(y \cdot \pi/L)\sin(z \cdot \pi/L),\nonumber\\
8t\sin(x \cdot \pi/L)\sin(y \cdot \pi/L)\sin(z \cdot \pi/L)\bigr)
\label{eq:motion}
\end{eqnarray}
